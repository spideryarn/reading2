 ...
 */
import { Gauge, Gavel, Hourglass, Lightbulb, ListChecks, MessageCircleQuestion, MessagesSquare, Network, RefreshCw, Rocket, ShieldCheck, type LucideIcon } from "lucide-react";
...
export function Dock({
  ...
  return (
    ...
    <TooltipGroup delay={{ open: 240, close: 90 }}>
      ...
      >
        {/* **THE SHARE COUNT, AND WHY IT IS A PROPERTY RATHER THAN AN
            ELEMENT.** `fit.ts` picks the bar's rung from `scrollWidth`, so
            anything that contributes width feeds back into the measurement and
            the ladder can oscillate — which is also why the overflow fades
            above are masks on `.dock` rather than child nodes. A custom
            property on an element that already exists adds no width at all.
            Agreed with session `decisions-mode`, 2026-09-09. */}
        <div
