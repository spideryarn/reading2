...
| [links.md](docs/project/links.md) | **hover the article's own hyperlinks**: what a card can say about a destination *without asking anybody*, which is where it goes, whether it leaves the publication, and whether it is a file rather than a page — the second of those being the fact a bare host cannot give you. Why an in-article `#fragment` is the one destination we can actually show, and why it is quoted rather than summarised; the three richer sources of card content that are deliberately not built yet and the order they are worth building in; why prefetching every link at ingest was rejected; and the measurement behind all of it — **13% of the links in this corpus have a glossary term as their link text**, which is why there is one card with two sections rather than two cards racing for the same three words |
