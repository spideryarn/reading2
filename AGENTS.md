...
| [performance.md](docs/project/performance.md) | **what the page costs when nobody is touching it**: the standard is Greg's — work a reader asked for may cost whatever it costs, work nobody asked for should cost nothing — and the three instruments, because measuring this went wrong twice before it went right. Mostly about **the confident wrong numbers**: a 60-second idle window that was 14 seconds of page load, `visibilityState` reading `hidden` on a tab plainly in front of you, a profiler attached to the top frame reporting a page carrying a YouTube embed as costing 0.0%, `ps` on macOS answering with a lifetime average that no amount of sampling moves, and a machine with 76 renderers where four unrelated tabs each outweigh the one you care about. Also the three culprits that died on measurement rather than argument — the embed costs 0.2% of a core, no CSS animation is running, and the reading view mounts **zero** job pollers whatever `useJobs.ts`'s own comment says — the four fixes that survived, why pausing the poll must not pause `drive` (the browser *is* the worker), and an honest list of what is still unmeasured and the one human step that unblocks it |
