...
| [reader-profile.md](docs/project/reader-profile.md) | **telling the model who is reading**: two boxes and one string, and why the previous version built both and read neither; where the profile rides in every prompt and why that is the *end* of it rather than the system prompt; the rules that live in `SYSTEM` whether or not anybody has a profile, and the forbidden sentence carried verbatim because a prompt ban relocates a register rather than deleting one; why the glossary is the case this is really for — a difficulty score is a property of the *pair*, not of the term; the three states of `profileHash` and why two of them are never stale; why `existingFor` is stricter than the staleness rule it sits beside; why the client may say *whether* but never *who* — and, since 2026-08-27, **the microphone that looked broken**: it was not, and the reason it looked it is that the device does not open until 1.1 seconds after the press while the button went orange immediately, and the one other piece of feedback was rendered only once it had something to say. The live level meter that fixes it, and the two decisions that make it believable — **one capture and never two**, because WebKit allows a single microphone source and a second one would draw the bars from a *different* microphone than the one being transcribed, and **nothing invented**, the fallback being the recogniser's own sound events rather than an animation. The floor that had to be measured rather than reasoned (a real quiet room is 10 dB below where you would put it, and the first guess pinned the meter at zero while everything worked), the four error codes that disarmed the button in total silence, and the accusation that was written and then deleted — because somebody who presses a button and then thinks looks exactly like a broken input device. And, hours later, **the same button still doing nothing, and still not being broken**: `getUserMedia({audio:true})` was handing the page a conferencing loopback that emits *exactly* `0.0` — digital silence, not a quiet room — so both instruments were right and neither was any use, because nothing said **which microphone** had produced the zero. Why the strip now names the device beside the threshold rather than joining them into a verdict, why the constraint is `exact` and why we refuse to guess which device you meant, why `MicOff` was the wrong glyph for a live microphone, why the timer's zero is `audiostart` and not the press — and **the audio kept when nothing came back**, whose trigger is deliberately not the one that was asked for, since "if there's an error" would have been silent through the whole failure that prompted it. Including the half that cannot be built at all: no web page can reveal a file in Finder |
...
| [diagram.md](docs/project/diagram.md) | **the article's shape as a picture**, in the band: four of them behind one toggle — `tree` is the outline, `force` is the relationships an outline cannot hold (sections joined by the distinctive words they share, over a structure built from the prose itself, with an honest account of what that measure does and does not know and why the words behind every link are shown rather than just the link), and `drift` and `trail` are **made of paragraphs rather than of sections**: one dot each, placed by an embedding of what the paragraph is *about* — `drift` keeps the article down the page and spends sideways on meaning (topic lanes, or one sliding scale), and `trail` is the only picture here that gives up down-the-page-is-later, because both of its axes are meaning and reading order is the line joining the dots. **What they can and cannot promise runs one way only** — two dots far apart really are far apart, two dots close together may differ entirely in what the projection threw away — which no percentage of variance rescues, so the strip says it in words. And, since 2026-08-27, **there were eight and four were cut**: what each of Strata, Mindmap, Arc and Cluster was a second way of drawing, why `strata` is the real loss (nothing here is to scale any more, and the question it answered — how much of the piece is that section — now falls to the spine), why the default is the only picture that is *free* since the other three each spend a model call the moment they are drawn, why every chip's hover card says what it costs as well as what it shows, why everything in Force that is not the reading-order chain got fainter without the ladder between them changing, and **the key that was firing twice** — a window-level handler and a focused widget both stepping the same ↑, one by a node and one by a section, and why `defaultPrevented` is the fix rather than a list of elements to skip. Plus the step buttons an iPad needs, which step by distinct start *row* rather than by node because a preorder walk would press ↓ three times and move the article nowhere. Also **the libraries the survey rejected and why** — every published diagram grammar spends width to show depth, and in a 288px band width is the axis we have not got — why the argument for hand-rolling was about the *data* rather than the algorithms and therefore changed when the data did, why a generated image of a structure is the [silent-success](docs/reusable/silent-success.md) pattern with a picture on it, and the secret the CSS and the TypeScript share, which is that SVG will not wrap text and moving a font size silently breaks arithmetic in another file |
...
| [performance.md](docs/project/performance.md) | **what the page costs, and how to find out** — opens with the commands, verbatim and verified: idle, scrolling, a real background tab, and a before/after against `main`. The standard is Greg's — work a reader asked for may cost whatever it costs, work nobody asked for should cost nothing — and the two halves are here in that order: the **poller and the clock that now stop when nobody is looking**, then the **scroll**, where the rail was re-rendering every band and tooltip sixty times a second to move one div, and `useColumnContext` was re-rendering the *whole table* to move three overlays (886 Spine renders across 30 seconds, down to 114). But mostly it is about **the confident wrong numbers**, because nearly every one on this page arrived plausible: a 60-second idle window that was 14 seconds of page load, `visibilityState` reading `hidden` on a tab plainly in front of you, a profiler on the top frame reporting a page carrying a YouTube embed as 0.0%, `ps` answering with a lifetime average no sampling moves, two Chromes silently sharing one debugging port so the second measured the first one's tab, a page that never rendered looking exactly like a fast one, and a Vite watcher on `data/` reloading the reader's article every time a test wrote a fixture — which made runs report **negative** CPU and was throwing away a real reader's page too. Hence the rule for reporting any of it: **render counts beat percentages**, identical code having varied 20% run to run while the counts held to 1%. Also why a clean main-thread profile is not evidence a page is idle, the culprits that died on measurement rather than argument, the local-Supabase `verifyOtp` that ended the "one human step" blocking every signed-in measurement, why a tab the user already had open is a **dead end** rather than a slow path, and GPT Sol's ranked list of what is left — including what it checked and found *not* worth doing |
...
| [counting-lines.md](docs/project/counting-lines.md) | **`npm run count-lines`**: how big this repo is, broken down by what each file is *for* rather than what language it is in — because cloc grouped by language gives a TypeScript repo one enormous row, and the questions worth asking are how much is product code, how much is tests, and how much is prose. The file list comes from `git ls-files` so that `.gitignore` is the only exclusion list anybody maintains, and the honest cost of that (a file you have not `git add`ed does not exist). Why a `.md` counts as prose wherever it sits, why fixtures and the lockfile are listed but kept out of the headline, and the three things it found by being run: **a symlink hid the repo's largest document**, because `CLAUDE.md` points at `AGENTS.md` and cloc deduplicates identical files without saying which one it dropped; cloc omitting `.jsonc` and `.gitignore` entirely rather than counting them as zero; and the arithmetic check that fires when a category goes missing, since a silently dropped bucket looks exactly like a repo that got smaller |
...
| [setup-dev.md](docs/project/setup-dev.md) | install, `npm run dev`, the command for each pipeline stage, and **which model each job uses** — two tiers in [`src/models.ts`](src/models.ts), capable (Sonnet 5) and quick (GPT-5.6 Luna), with a table saying which job is on which and everything on capable for now; the two spellings the Anthropic SDK and OpenRouter each want, and why the quick tier can only have one of them — and, since 2026-08-27, **the third spelling, which is the one a person reads**: `/profile` printed ten jobs' raw wire ids, so seven said `claude-sonnet-5` and three said `anthropic/claude-sonnet-5`, every string correct and the page as a whole misleading, because a reader counting names counts two models. A provider prefix is an address, not a name. Why the display table is literals rather than a `split("/")` (the *previous* model pair would have stripped to `claude-sonnet-4.5` against Anthropic's `claude-sonnet-4-5` and gone on disagreeing while looking mended), why an unlisted model shows its raw id on purpose, and why there is now exactly **one** public task-level model function rather than a similarly-named pair one of which would cheerfully answer for a stage that has never spoken to OpenRouter. Also **the override that the page could not see** — `SPIDERYARN_*_MODEL` was read at the three call sites and nowhere else, so a card promising *what the server is configured with* named a model the server was not using — the two models on no tier that an inventory claiming to be complete had quietly omitted, and **the pin that will fail in silence the day anybody moves a request-path job**: an Anthropic provider preference on a request no Anthropic upstream can serve, which OpenRouter answers correctly and charges for |
