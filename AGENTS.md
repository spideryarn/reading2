...
| [deployment.md](docs/project/deployment.md) | **Spideryarn on Vercel**: the new project and why it is not the old one, `vercel deploy` from a working tree because Vercel is not connected to the git remote, and why the build compiles the API itself — Vercel uses this repo's TypeScript 7, which its builder cannot drive, and then *reports success and ships a function that fails on every request*. Also: why there is deliberately no `spideryarn.vercel.app` (Pro cannot protect a production domain, and that one counts as one), what `/api/health` is for and why it is hard to please, the four things here that fail without saying so, and an honest list of what does not work in production yet |
