...
| [diagram.md](docs/project/diagram.md) | **the article's shape as a picture**, in the band: three of them behind one toggle, and the reason they are three rather than one — a map that is faithful about *size* cannot also be legible about *names*, so `strata` is to scale (and answers the one question the gist columns cannot: how much of the piece is that section) while `tree` and `mindmap` give every name the room it needs. Mostly about **the twenty libraries the survey rejected and why** — every published diagram grammar spends width to show depth, and in a 288px band width is the axis we have not got; why d3-hierarchy's own manual talks you out of the two functions you would import it for; why a generated image of a structure is the [silent-success](docs/reusable/silent-success.md) pattern with a picture on it; why the detail goes in a footer strip rather than a hover card when a hover target is 6px tall; and the secret the CSS and the TypeScript share, which is that SVG will not wrap text and moving a font size silently breaks arithmetic in another file |
