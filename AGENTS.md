...
| [diagram.md](docs/project/diagram.md) | **the article's shape as a picture**, in the band: six of them behind one toggle, in two groups. Three hand-rolled ones draw the **tree**, and the reason they are three rather than one is that a map faithful about *size* cannot also be legible about *names* — `strata` is to scale (in words, and it answers the one question the gist columns cannot: how much of the piece is that section) while `tree` and `mindmap` give every name the room it needs. Three D3 ones draw a **graph** — and, since 2026-08-27, that graph has **five kinds of edge rather than three**, arranged as a ladder by how much each one knows: the tree's containment and its reading order (now one chain through the whole article rather than a list of siblings, drawn thick with an arrow, and the arrowhead has to stop at the circle's edge or it is painted underneath it); the article's **own cross-references**, which are the only line in the picture that somebody *meant*, and which six of our seven articles do not have at all; tf-idf term overlap; and an **embedding model's opinion**, drawn dotted because it is the only one that costs money and the only one that can be confidently wrong. With an honest account of what a lexical measure does and does not know, why the words behind every link are shown rather than just the link, and **the scale trap that would have silently ruined the experiment** — an embedding cosine runs 0.6–0.9 where a tf-idf cosine runs 0.12–0.5, so letting the two share a force strength makes the new edges *become* the picture's shape rather than adding to it. Also **the libraries the survey rejected and why** — every published diagram grammar spends width to show depth, and in a 288px band width is the axis we have not got — why the argument for hand-rolling was about the *data* rather than the algorithms and therefore changed when the data did, why a generated image of a structure is the [silent-success](docs/reusable/silent-success.md) pattern with a picture on it, and the secret the CSS and the TypeScript share, which is that SVG will not wrap text and moving a font size silently breaks arithmetic in another file |
