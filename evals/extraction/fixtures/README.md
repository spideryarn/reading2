...
| `archwiki_install.html` | [wiki.archlinux.org/title/Installation_guide](https://wiki.archlinux.org/title/Installation_guide) | **GNU Free Documentation License 1.3 or later** — quoted directly from the page's own footer: *"Content is available under GNU Free Documentation License 1.3 or later unless otherwise noted."* Checked per Greg's instruction rather than assumed; this page is GFDL, not CC BY-SA | **Thirteen** admonition boxes (`class="archwiki-template-box"` plus `-note` ×7, `-tip` ×5, `-warning` ×1), each labelled by a bare `<strong>` first child with no titled element and no attribute, so the *visible word* is the only thing that looks like a signal — which is what makes it the corpus's hardest case for "recognise by markup, never by words". This row used to say *twelve `Note`/`Tip` boxes*, which was **incomplete rather than wrong** (7 + 5 = 12; it left the `-warning` out of the population), and it said the label sits before a sibling `<ul>`, which **was wrong**: that shape is **4 of the 13**, the other 9 carrying their label inline. Corrected 2026-09-07. Still the first MediaWiki instance in this corpus that isn't Wikipedia itself. What reaches a reader is asserted in `tests/callouts.test.ts` rather than restated here |
