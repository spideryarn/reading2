...
async function runWith(run: () => Promise<UsageReport>, forMs = 400): Promise<string> {
  const root = tempRoot();
  ...
  await runOverseer({
    ...
    usage: { intervalMs: 120, run, onPass: retention.onPass },
  });
  retention.close();
  return root;
}
...
describe("a usage pass becomes a line on disk", () => {
  ...
  test("nothing is written when no pass ever runs", async () => {
    ...
    await runOverseer({
      ...
      usage: { intervalMs: 100_000, run: async () => report(), onPass: retention.onPass },
    });
    retention.close();
