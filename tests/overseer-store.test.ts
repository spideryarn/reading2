...
/**
 * The usage report the checkpoint carries, and the three ways it can carry none.
 *
 * The pairing with `attention` is the point of most of these: a wait cannot
 * survive a gap in observation and a rate-limit rejection can, so one is deliberately
 * NOT restored across a restart and the other deliberately IS.
 */
describe("the usage report the checkpoint holds", () => {
  test("a fresh store says no pass has run, rather than that nothing is wrong", () => {
    const root = tempRoot();
    const store = mustOpen(root, () => new Date("2026-09-08T10:00:00.000Z"));

    const written = store.checkpoint({ lastGoodSnapshotAt: null, tick: true });

    expect(written.ok).toBe(true);
    if (!written.ok) throw new Error("unreachable");
    // NOT a report saying no limits were found: that is a claim, and it is the
    // most reassuring possible lie for a probe that has never looked.
    expect(written.checkpoint.usage.kind).toBe("none");
    if (written.checkpoint.usage.kind !== "none") throw new Error("unreachable");
    expect(written.checkpoint.usage.why).toMatch(/no usage pass has run/);
    expect(written.checkpoint.usage.at).toBe("2026-09-08T10:00:00.000Z");
  });

  test("a checkpoint written before the field existed reads as none, with that reason", () => {
    const root = tempRoot();
    const store = mustOpen(root);
    store.checkpoint({ lastGoodSnapshotAt: null, tick: true });
    store.close();
    // Strip the field the way a checkpoint from an older build would not have had it.
    const path = join(root, CHECKPOINT_FILE);
    ...
    if (read.checkpoint.usage.kind !== "none") throw new Error("unreachable");
    // A DIFFERENT sentence from "no pass has run": one says the writer was older,
    // the other says nothing has looked. They are not the same fact.
    expect(read.checkpoint.usage.why).toMatch(/written before the Overseer had one/);
  });

  test("a malformed stored report degrades to none and does NOT fail the checkpoint", () => {
    const root = tempRoot();
    const store = mustOpen(root);
    store.append([seenEvent(observedRow(), "2026-09-08T10:00:00.000Z")]);
    store.checkpoint({ lastGoodSnapshotAt: null, tick: true });
    ...
    const path = join(root, CHECKPOINT_FILE);
    const raw = JSON.parse(readFileSync(path, "utf8")) as Record<string, unknown>;
    raw["usage"] = { kind: "report", report: { nonsense: true } };
    writeFileSync(path, `${JSON.stringify(raw, null, 2)}\n`);

    const read = readCheckpoint(root);

    // The whole point of degrading: the REGISTER survives, because it is the thing
    // there is no second copy of. The report is regenerated by the next scan.
    expect(read.kind).toBe("checkpoint");
    if (read.kind !== "checkpoint") throw new Error("unreachable");
    expect(read.checkpoint.register).toHaveLength(1);
    expect(read.checkpoint.usage.kind).toBe("none");
    if (read.checkpoint.usage.kind !== "none") throw new Error("unreachable");
    expect(read.checkpoint.usage.why).toMatch(/unusable/);
  });

  test("a stored none arm survives a round trip with its own reason and instant", () => {
    const root = tempRoot();
    const store = mustOpen(root);
    store.checkpoint({
      lastGoodSnapshotAt: null,
      tick: true,
      usage: { kind: "none", why: "OPENROUTER_API_KEY is not set, so no scan was attempted", at: "2026-09-08T11:00:00.000Z" },
    });
    store.close();
    ...
    expect(read.kind).toBe("checkpoint");
    if (read.kind !== "checkpoint") throw new Error("unreachable");
    expect(read.checkpoint.usage).toEqual({
      kind: "none",
      why: "OPENROUTER_API_KEY is not set, so no scan was attempted",
      at: "2026-09-08T11:00:00.000Z",
    });
  });

  test("a write carrying no report keeps the one the store already holds", () => {
    const root = tempRoot();
    const store = mustOpen(root);
    const held = { kind: "none", why: "the scan was skipped this pass", at: "2026-09-08T11:00:00.000Z" } as const;
    store.checkpoint({ lastGoodSnapshotAt: null, tick: true, usage: held });

    // A scan costs 30-45 seconds over ~2.9 GB, so most ticks carry no new report.
    // Holding the old one is right: an account has not stopped being rate-limited
    // because nobody looked.
    const second = store.checkpoint({ lastGoodSnapshotAt: null, tick: true });

    expect(second.ok).toBe(true);
    if (!second.ok) throw new Error("unreachable");
    expect(second.checkpoint.usage).toEqual(held);
  });

  test("a restart RESTORES the report, where it deliberately does not restore the attention list", () => {
    const root = tempRoot();
    const first = mustOpen(root);
    first.append([seenEvent(observedRow(), "2026-09-08T10:00:00.000Z")]);
    const held = { kind: "none", why: "a real scan would go here", at: "2026-09-08T11:00:00.000Z" } as const;
    first.checkpoint({ lastGoodSnapshotAt: "2026-09-08T10:00:00.000Z", tick: true, usage: held });
    first.close();

    const second = mustOpen(root, () => new Date("2026-09-08T12:00:00.000Z"));
    const written = second.checkpoint({ lastGoodSnapshotAt: null, tick: true });

    expect(written.ok).toBe(true);
    if (!written.ok) throw new Error("unreachable");
    // RESTORED: a rejection is a fact about text with its own resetsAt, true on
    // both sides of a restart, and re-finding it costs a 2.9 GB read.
    expect(written.checkpoint.usage).toEqual(held);
    // NOT restored, and the contrast is the rule: a wait is about continuous
    // observation, so a first-seen instant cannot survive a gap we were absent for.
  ...
  test("a rebuilt start does not inherit a report it could not read", () => {
    ...
    first.checkpoint({
      lastGoodSnapshotAt: null,
      tick: true,
      usage: { kind: "none", why: "should not survive an unreadable checkpoint", at: "2026-09-08T11:00:00.000Z" },
    });
    first.close();
    // The checkpoint is destroyed; the log is not. The store rebuilds from events.
    writeFileSync(join(root, CHECKPOINT_FILE), "{ this is not json\n");
    ...
    expect(written.ok).toBe(true);
    if (!written.ok) throw new Error("unreachable");
    if (written.checkpoint.usage.kind !== "none") throw new Error("expected none");
    // "No pass has run", not the sentence from the checkpoint we could not read:
    // a rebuild has no report and must say so rather than inherit one.
    expect(written.checkpoint.usage.why).toMatch(/no usage pass has run/);
  });
});
