...
const ALLOWED: Readonly<Record<string, string>> = {
  ...
  "tools/fleet/describe.ts":
    "THE FLEET DASHBOARD'S DESCRIBER, and the same argument as the Overseer's seam above, one directory along. It buys real inference — one `openai/gpt-5.6-luna` call per session opening, and one per idle session whose tail changed — and it cannot go through src/ai-call.ts, because tools/fleet must run with the product's server absent; importing the seam would pull the app and pg/drizzle/stripe/jsdom into a box utility. Greg decided this shape for this file explicitly (2026-09-09, quoted in its header). It is not a declared bypass either: a Declaration needs a `job: AiJob`, and AiJob is the closed union of the calls THIS APP pays for — `fleet-dictation` could borrow \"dictation\" because the product has that job, and describing an agent session has no member to borrow. Minting one would push a non-app job into AI_JOB_WIRE and every table keyed off it. So it is here and in UNMETERED_SPEND, because a green test is not a register.",
