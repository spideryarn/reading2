...
Five articles, 56 files, **1.14 MB**, tracked in git. This is the thing a fresh clone has been
...
| `openai-huggingface` | 95 | 176 KB | The only carrier anywhere in `data/` of **`quotes.json`, `timeline.json` and `assets.json`**. [`store-roundtrip`](../../store-roundtrip.test.ts) has a test whose whole job is to notice a corpus that has quietly stopped covering an artefact; without this slug three of its rows would assert only that the export invented nothing. Also **the article carrying a `supplement` block**, which is what [`store-shelf-reads`](../../store-shelf-reads.test.ts) seeds `test-shelf-corpus-supplement` from — it is the only one today, and the guard is written about this slug by name rather than about whichever slug happens to have one. | **Carries those three artefacts.** And **one `supplement` block of more than zero words**, so `body` word count differs from `total`. `wordCount` became *body* words on 2026-08-28 and for three days the scalars audit recomputed the total instead — invisible, because the only article in the database had no supplement block and the two "agreed by having nothing to disagree about" ([260831b](../../../docs/postmortems/260831b-the-audit-that-recomputed-the-old-definition.md)). Lose this block and that audit is vacuous again with every suite still green. |
