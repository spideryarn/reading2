...
describe("fleetState — the one wire shape", () => {
  ...
  it("distinguishes a stalled collector from a quiet box", () => {
    ...
    const stalled = fleetState({ ...snap, collectedAt: stale }, null, null, 60_000, true, null, NOT_ASKED, NO_OVERSEER, NO_USAGE);
    ...
    const trying = fleetState({ ...snap, collectedAt: stale }, null, null, 60_000, true, now, NOT_ASKED, NO_OVERSEER, NO_USAGE);
  ...
  it("reads a payload this file actually produced, rather than a hand-built one", () => {
    ...
    const live = JSON.parse(
      JSON.stringify(fleetState(snap, null, null, 60_000, true, "2026-09-08T03:31:00.000Z", NOT_ASKED, NO_OVERSEER, NO_USAGE)),
  ...
  it("says NEVER COLLECTED with a null clock rather than an empty box", () => {
    ...
    const s = fleetState(null, null, null, 60_000, false, null, NOT_ASKED, NO_OVERSEER, NO_USAGE);
  ...
  it("never invents a timestamp for a collection that did not happen", () => {
    ...
    expect(fleetState(null, null, null, 60_000, false, null, NOT_ASKED, NO_OVERSEER, NO_USAGE).collectedAt).toBeNull();
  });
  ...
  it("keeps the previous rows and clock when a refresh failed", () => {
    ...
    const s = fleetState(snap, "tmux: connection refused", null, 60_000, false, null, NOT_ASKED, NO_OVERSEER, NO_USAGE);
  ...
  it("carries the refresh interval, so nothing has to guess when it is late", () => {
    ...
    expect(fleetState(snap, null, null, 60_000, false, null, NOT_ASKED, NO_OVERSEER, NO_USAGE).refreshMs).toBe(60_000);
  });
  ...
  it("tells the page whether answering is switched on, rather than leaving it to guess", () => {
    ...
    expect(fleetState(snap, null, null, 60_000, false, null, NOT_ASKED, NO_OVERSEER, NO_USAGE).answeringEnabled).toBe(false);
    expect(fleetState(snap, null, null, 60_000, true, null, NOT_ASKED, NO_OVERSEER, NO_USAGE).answeringEnabled).toBe(true);
  });
});
