 ...
 *
 * The second was the iPad, and it is the reason `bandCovers` is a fact from
 * layout.ts rather than a width compared here. Portrait is **834px**, which was
 ...
 *
 * **The iPad has since stopped being a case at all**, and the plumbing that
 * argument bought is what made that free: the crossover moved from 844 to 700
 * on 2026-09-06 (layout.ts § `MODE_PROSE_FLOOR`) and every width in this file
 * followed it without a line of it being recompared, because none of them is a
 * width. An iPad in portrait now gets the band beside the article. Keeping the
 * story is the point — the next move of that number will be as quiet.
 *
...
import { bandCoversProse, MODE_MIN, MODE_PROSE_FLOOR, SPINE_W } from "../src/web/layout.js";
import {
  moreRoomSideways,
  readMachine,
  rememberDismissed,
...
describe("the crossover the banner describes", () => {
  it("is layout.ts's, and moves with the rail", () => {
    // 688 with `?spine=0`, 700 without it. Written out as the two boundaries
    ...
    //
    // **`MODE_PROSE_FLOOR`, not `PROSE_MIN`, since 2026-09-06.** The band yields to
    // the prose down to 544 while there is room to give; the question here is
    // the different one of how narrow a reading column is still worth standing
    // beside, and that is the floor — layout.ts § `MODE_PROSE_FLOOR`.
    expect(covers(MODE_MIN + MODE_PROSE_FLOOR - 1, false)).toBe(true);
    expect(covers(MODE_MIN + MODE_PROSE_FLOOR, false)).toBe(false);
    expect(covers(MODE_MIN + MODE_PROSE_FLOOR + SPINE_W - 1)).toBe(true);
    expect(covers(MODE_MIN + MODE_PROSE_FLOOR + SPINE_W)).toBe(false);
  });
  ...
  it("treats an unset rail as an on rail, the way an opening band will find it", () => {
    ...
    // 694 is inside the twelve pixels the rail is worth: 694 − 12 = 682 is
    // under the combined `MODE_MIN + MODE_PROSE_FLOOR` minimum of 688, and 694
    // itself is over it — the divergent interval is exactly [688, 700). It is
    // not "under the floor": the floor is 400, and the sum is what a window is
    // measured against. Any width outside that gap
    // answers alike whatever the rail is doing and would prove nothing.
    expect(covers(694, null)).toBe(true);
    expect(covers(694, true)).toBe(true);
    expect(covers(694, false)).toBe(false);
  });

  it("leaves an iPad alone in both orientations, since 2026-09-06", () => {
    // 834 × 1194 and back again. **Portrait used to warn**, and this is the
    // case that made `bandCovers` a fact rather than an approximation — see the
    // header, which is still the reason the plumbing is shaped this way. But
    // the crossover it compares against moved from 844 to 700 when the prose
    // got a floor separate from its defended width (layout.ts § `MODE_PROSE_FLOOR`),
    // and 834 clears 700 with 134px to spare: an iPad in portrait now gets the
    // band *beside* the article — 288 of band against 534 of prose — so there
    // is nothing to warn about. The banner is a phone's now, which is what the
    // rest of this file is about.
    expect(shouldWarnSmallScreen({ ...PHONE, bandCovers: covers(834) })).toBe(false);
    expect(shouldWarnSmallScreen({ ...PHONE, bandCovers: covers(1194) })).toBe(false);
  });

  it("gives way at 700 exactly, on a window with nothing taken out of it", () => {
    // 700 − 12 of rail = 688 = `MODE_MIN + MODE_PROSE_FLOOR`: the band fits beside
    // the prose with nothing to spare. Pinned because it is the single width at
    // which a `>=` written for a `>` would be invisible.
    //
    // **This is arithmetic, not a device.** It is tempting to name a phone
    // here, and that would be false: `useWindowWidth` in App.tsx hands
    // `bandCoversProse` the width *minus the notch*, and a notched phone
    // sideways spends tens of pixels on safe-area insets — so a real 700pt
    // landscape phone is fed something well under 700 and keeps the banner. The
    // runtime behaviour is right either way; the false thing would be the name.
    // GPT Sol, 2026-09-05. What the 2026-09-06 move to `MODE_PROSE_FLOOR` bought is
    // that a *modern* phone sideways (≈734 after its insets) now clears this
    // and gets the band beside the article, which is the point of the change —
    // but naming one on this line would still be the same mistake.
    expect(shouldWarnSmallScreen({ ...PHONE, bandCovers: covers(390) })).toBe(true);
    expect(shouldWarnSmallScreen({ ...PHONE, bandCovers: covers(700) })).toBe(false);
