...
| `SPIDERYARN_CITATIONS_FIND_MODEL` | Citations mode's *Find it*, the call that runs one web search for a cited work and picks which result, if any, is its own page ([citations.md](citations.md#find-it-on-the-web)) |
...
| `npm run hierarchy -- <slug> [--force]` | 4, the tree and an **empty** labels manifest ([hierarchy.md](hierarchy.md)). One model pass since 2026-09-06 — the structure — and the labels are the row below | Writing a pending manifest **deletes this revision's `labels` receipt**, so running this always makes the labels step runnable again. **It does not then run them, and at a terminal nothing else will** — publication queues the free labels job, a browser's `jobEngine` drives every queued job its owner has, and this command drives only its own. So a terminal-only article sits at *"Paragraph labels are still arriving"* until you run the row below; the command says so when it finishes. Greg's call, 2026-09-07, over having this command spend the label time again. A forced re-run still replays the structure out of the article's checkpoints and buys nothing new (measured 2026-09-05); judging a *structure*-prompt change is `npm run eval:hierarchy-structure` |
