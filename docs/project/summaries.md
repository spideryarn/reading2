...
every article that has been through the ToC stage.
...
every internal node of the tree ([`src/toc.ts`](../../src/toc.ts)), and this panel is those gists,
...
- **A section whose gist is subtly wrong** is undetectable from here. Nothing catches a plausible
  ...
  [table-of-contents.md](table-of-contents.md), so that is where such a check would belong.
...
- [granularity-zoom.md](granularity-zoom.md) — the tree this panel draws, and why detail and depth are
  one axis
- [table-of-contents.md](table-of-contents.md) — stage 4, which writes every gist on screen here
