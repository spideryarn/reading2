...
| `/?mode=structure` | **two faces**: two linked columns where the band's border box is at least 389px, the nested list where it is narrower. Resize across the threshold and check it swaps without flicker, and that list titles wrap rather than being cut to one line |
