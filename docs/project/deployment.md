...
| `STRIPE_PRICE_READER` | the `price_…` the paid tier is sold at. **Not a secret** — it is in the Checkout URL every customer sees. Test and live mode have different ids, so the live one is created at go-live by `scripts/stripe-setup.ts` and set here |
