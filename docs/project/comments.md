...
so a block whose words changed can be re-minted and the comment on it then points at an id no block
carries. What keeps the comment itself is not the id at all: it is stored against the article rather
than a revision. Worth stating because the tidier version — *pinned to the permanent id, so the
comment survives* — has the facts right and the causation wrong, which reads as true and got as far
as a reader-facing tooltip on 2026-09-07 before a review caught it
([260907b § Stage 2](../plans/260907b-rich-tooltips-on-the-dock-modes.md)). Click it and the dialog opens on the block's first comment in reading order.
