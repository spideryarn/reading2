...
| [`src/web/Dock.tsx`](../../src/web/Dock.tsx) | the bottom bar and the drawer that rises out of it: the mode switch (ten of them, `MODES_UI`), your questions, and the links to the tweets and metadata pages — [bottom-bar.md](../plans/bottom-bar.md). Its buttons are **three** kinds — navigate, open a drawer, switch mode — and the markup says which. The order is Greg's, set by hand; the way home and the dimmed placeholders both left it on 2026-08-26 |
