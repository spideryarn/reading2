...
| [`src/web/modes/`](../../src/web/modes/) | a mode's controller and its band, out of `App.tsx` — `modes/ideas/IdeasMode.tsx` was the first, joined on 2026-09-06 by Timeline, Quotes, Debate and Glossary, then by Search, Summary, Diagram and Referee, and more follow under [260905e](../plans/260905e-main-app-architecture-review.md). The extraction is what lets a boundary enclose the mode's own computation, since a boundary cannot catch a throw from the component that renders it |
