...
gate 3's last bullet made mechanical rather than remembered. **The documents are re-read on every
tick**, not once at start: a session is told to follow the file on disk, so a digest taken when the
daemon started would have let an edit run under the old pin until the next restart
