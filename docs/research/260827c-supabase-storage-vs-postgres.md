...
Researched for the Postgres-migration plan ([260825f-postgres-migration.md](../plans/260825f-postgres-migration.md)):
...
- **`pg_net`** (Supabase's async HTTP extension) can fire an HTTP GET at a Storage URL, but it's
  ...
  **unlogged** table (lost on crash). Source:
  ...
  > reviewing [260827o-raw-bytes-in-storage.md](../plans/260827o-raw-bytes-in-storage.md). The conclusion does not
