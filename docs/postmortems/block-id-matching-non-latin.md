...
**2026-08-26.** Found while planning [PDF ingestion](../plans/pdf-ingestion.md), not from a report.
...
[deterministic-block-ids.md](../plans/deterministic-block-ids.md) — which calls `matchKey` "a
...
- **When a name generalises but the code does not, believe the code.** `normalize` promises a
  category. The regex names an alphabet. [toc-max-tokens.md](toc-max-tokens.md) ends on the same
...
- [deterministic-block-ids.md](../plans/deterministic-block-ids.md) — the road not taken, which read
  this same function closely and did not see this
- [pdf-ingestion.md](../plans/pdf-ingestion.md) — the work this blocks
- [silent-success.md](../reusable/silent-success.md) — the pattern
- [toc-max-tokens.md](toc-max-tokens.md) — the ASCII-fixture observation, from the other end
