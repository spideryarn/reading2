...
Worse, the demolition leaves open dropping `raw_content_type`, `raw_encoding`, and `raw_sha256` (`docs/plans/delete-the-importer.md:153-160`). `raw_sources` does not replace their meanings:
...
The plan says existing rows remain (`docs/plans/delete-the-importer.md:63-72`) and also simplifies the gate on the premise that there are “no legacy rows” (`docs/plans/delete-the-importer.md:45-54`). Those statements conflict.
...
The demolition list rewrites `db:export` but never mentions `sendSource` (`docs/plans/delete-the-importer.md:151-161`).
...
The plan has thought about the checkpoint store, but has not decided it. D removes `dir`, while label progress and PDF chunks deliberately survive failed attempts (`docs/plans/transactional-stage-runner.md:72-80`, `424-426`).
