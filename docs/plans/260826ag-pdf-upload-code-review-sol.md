...
There is one more serious conflict in the plan: it says an existing hash means “transfer nothing” and immediately creates a reader’s article (`docs/plans/260826u-pdf-upload-and-storage.md:558`). If that decision is based on the browser’s claimed SHA, the hash becomes a capability: creating the article reference may grant later source-read access. That contradicts “a hash is not a grant” at `docs/plans/260826u-pdf-upload-and-storage.md:585`.
