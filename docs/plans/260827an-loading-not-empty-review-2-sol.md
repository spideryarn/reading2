...
2. **should-fix — the most delicate new paths remain untested.** The new hook test covers `useComments`, but nothing tests `useChat`’s boolean-returning `refresh`, StrictMode duplication, `useSearch.loadFailed`, Search’s failure branch, or its delayed indicator. This is why finding 1 passes all current tests. The plan overstates coverage when it says each panel file covers all five states ([260827an-loading-not-empty.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/260827an-loading-not-empty.md:107)).
