...
2. **CRITICAL — C7 cannot use `copyArtefacts` with the proposed Postgres adapter protocol. Confidence: high.**
   ...
   `copyArtefacts` calls only `write` (`tests/helpers/artefacts.ts:87-91`). It never calls `beginStep` or `finishStep`. But C4 defines Postgres `has` as requiring a done run row, and C2 makes completion exclusively the fenced `finishStep` transition (`docs/plans/260827aa-delete-the-importer.md:401-425`).
   ...
   The missing fixture coordinator cannot be deferred to D, because the plan explicitly excludes that coordinator from C while expecting C7 green (`docs/plans/260827aa-delete-the-importer.md:441-447`).
...
6. **HIGH — C2’s proposed red test stays green after removing the attempt fence. Confidence: high.**
   ...
   The test starts from “a step-run that has ended” (`docs/plans/260827aa-delete-the-importer.md:408-410`). Such a row is already not `status = 'running'`. Delete `attempt_id = ?`, and the remaining status predicate still refuses it.
