<!-- GPT Sol's review of auth-ui-and-production.md, 2026-08-26. Kept verbatim except
     ...
     auth-ui-and-production.md § The cross-family review. -->
...
The plan records that `DATABASE_URL` is unset (`auth-ui-and-production.md:96`), but never adds it to the work. It even says “Nothing else” after the two auth dashboards (`auth-ui-and-production.md:516`). The production checklist tolerates health remaining broken “once DATABASE_URL is set” (`auth-ui-and-production.md:561`).
...
The release order also needs an explicit fence. Missing `VITE_*` values deliberately throw during module loading (`auth-ui-and-production.md:152`); deploying before those values, Google enablement, and the Google redirect registration produces a blank client or an API gate with no usable sign-in route. A push to `main` deploys automatically (`deployment.md:37`).
...
The test at `auth-ui-and-production.md:269` passes while `/auth/callback?code=X` remains forever on screen.
...
- The old specification still maps every `getClaims` failure to 401 (`auth-supabase.md:490`), despite the earlier review requiring 503 for JWKS/network unavailability. Otherwise clients refresh valid sessions pointlessly and report infrastructure failure as bad credentials.
- The tests still require a “wrong email → 403” (`auth-supabase.md:782`), while `isAllowed` deliberately accepts every authenticated email. That test cannot coexist with the decision.
