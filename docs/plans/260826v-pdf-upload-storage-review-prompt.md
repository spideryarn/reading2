...
Written 2026-08-26 for [260826u-pdf-upload-and-storage.md](260826u-pdf-upload-and-storage.md), and **not yet
...
npx tsx scripts/run-codex.ts --model gpt-5.6-sol --effort high --timeout-minutes 45 \
  --prompt-file docs/plans/260826v-pdf-upload-storage-review-prompt.md \
  --output docs/plans/260826z-pdf-upload-storage-review-sol.md
```
...
`docs/plans/260826u-pdf-upload-and-storage.md` — read it in full first.

It is step 7 of `docs/plans/260826c-pdf-ingestion.md` (see its "### Upload" section and "## Build order").
Read those two sections of `260826c-pdf-ingestion.md` too, and `docs/plans/260825f-postgres-migration.md`'s schema
section plus `docs/plans/260826e-postgres-storage-implementation.md` where it discusses `raw_bytes`.
...
- `src/db/schema.ts` (built) has `raw_bytes bytea` on `article_revisions` — the fetched bytes in
  the database row.
- `docs/plans/260826c-pdf-ingestion.md` says an uploaded PDF's bytes live in a Supabase Storage object,
