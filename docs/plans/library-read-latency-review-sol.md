...
The claim that nothing changes `tree` or `revision_blocks` after publication is false ([plan](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/library-read-latency.md:302)).
...
`headingTitle` is placed inside `REVISION_PROJECTIONS.library` ([plan](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/library-read-latency.md:149)), but it corresponds to no `article_revisions` column. The proposed policy test derives its complete expected key set only from revision columns, so it must reject `headingTitle`.
