# Re-review: one title rule for the server and the client (round 2)

You reviewed this change and returned **CHANGES REQUESTED** with two findings and a nit. All three
are addressed below. This round should decide whether the change is fit to commit, and whether the
fixes introduced anything new — particularly in the transport, which is the part that now parses a
string a stranger controls.

## Your finding 1 [P2, blocking] — non-default modes change the title after mount

Fixed, the way you suggested first: **the server learns the mode.** The client's rule (the mode
distinguishes tabs, so it belongs in the title) is the one with the argument behind it, so it stands.

- `readMode(url)` in `src/vercel.ts` reads `?mode=` off the restored URL and resolves anything
  unrecognised to the default through `isMode`, which is the same predicate `modeParam` uses on the
  client — one place decides what a mode is. It is deliberately tolerant of a malformed URL: a throw
  there would be a 500 on an address that only wanted a tab title.
- `servePublicReadPage` takes the resolved mode; `composeShell(shell, head, mode)` passes it to
  `documentTitle`.
- **The mode reaches `<title>` only.** `og:title`, `twitter:title` and `og:url` are unchanged — a
  shared link is about the article, not about which panel the sharer had open.
- `MODES`/`Mode`/`DEFAULT_MODE` moved from `src/web/params.ts` to a new leaf `src/modes.ts`, because
  nothing the function reaches may import from `src/web/`. `params.ts` re-exports all three, so no
  component knows it moved. `MODE_LABEL` moved to `src/title-text.ts` beside `documentTitle`.

Your sentence — *"the missing dimension is title state, not title characters"* — is quoted in
`docs/project/page-titles.md` and in the test, because the general lesson about corpora is worth more
than the bug.

`tests/page-head.test.ts` now loops over `MODES` read from `src/modes.ts` (so a tenth mode arrives in
the check unbidden), plus spelled-out literals for the default and one non-default mode so the loop
cannot be satisfied by a function that labels everything. `tests/public-read-rewrite.test.ts` covers
`readMode` including the round trip through `originalUrl`, since a rewrite that dropped the query
would otherwise pass against a hand-written URL.

## Your finding 2 [P2, test gap] — the first-`h1` fallback has two unpaired implementations

Fixed. New fixture `a public article whose only title is its first <h1>`: `title: null`, an `h2` at
ordinal 0, `h1`s at ordinals 1 and 2, and **the rows inserted out of ordinal order**. It compares
`loadHead.title` against `/api/public/article/:slug`'s `meta.title` and against a spelled-out
expected first `h1`.

Your two named mutations were run and both go red on the pairing assertion:

- `level = 1` → `level = 2` in `PUBLIC_HEADING_TITLE`:
  `expected 'A subheading that comes first' to be 'The first level-one heading'`
- `order by ordinal` → `order by ordinal desc`:
  `expected 'A second level-one heading, later in …' to be 'The first level-one heading'`
- and the other side, `headingTitleOf` dropping its `level === 1` check:
  `expected 'The first level-one heading' to be 'A subheading that comes first'`

## Your nit — `normaliseText`'s "nothing invisible" claim

Corrected. `src/html.ts` now says what it does (bidi controls, C0/C1, and the `\s` collapse) and
states explicitly that U+200B and U+2060 survive, and that stripping them would be a separate
decision. The fuzz's alphabet comment no longer groups U+200B with whitespace `\s` reaches.

## What I want from this round

1. **The transport.** `readMode` parses a stranger's query string. Is there an input that makes it
   throw, or that gets a mode past `isMode` that the client would resolve differently? The two must
   agree on *every* string, not just on the nine good ones — a divergence there is the same bug in a
   new place.
2. **Is there a fifth?** Two of the four were found by you and neither by any test. The shape is *two
   sources answering one question about the tab*. I have checked description, canonical, `og:title`
   and `og:url` (no second writer), and the `view` axis (`/read/x/metadata` is two segments, so the
   rewrite never composes it — please confirm that reasoning).
3. **The `src/modes.ts` extraction.** `params.ts` now imports and re-exports. Did anything about the
   mode vocabulary change behaviour, and is the re-export shape right?
4. Whether this is fit to commit.

## Evidence

Mutations run this round, each red on the assertion named for it and no other:

- `readMode` always returns the default → the two `readMode` cases
- `documentTitle` ignores the mode → the all-modes loop and the spelled-out pair
- the default mode is labelled too → the same two, plus every literal title case
- the mode leaks into `og:title` → the card cases
- and the three h1 mutations above

`npm run typecheck`: clean on all three projects. Both builds pass (client, then the API bundle,
which re-runs the stale-shell refusal). `check-public-shell.ts --self-test`: 46/46.

`npm test`: 5976 passed, 6 failed. The failures are `tests/auth-callback.test.ts` (a supabase mock
missing `onAuthStateChange`, which ten other test files mock), `tests/admin-store.test.ts` and
`tests/store-jobs-parity.test.ts`. **All pass when run alone**, and the failing set moves between runs
(3, 6 and 10 failures across four runs, different files each time, both before and after this
change). It is the repo's known condition: several pg suites run in parallel against one database and
`admin-store` truncates `spideryarn.articles` while other suites' fixtures are live. I did not make it
worse and I have not tried to fix it; say if you think this change is implicated.

## The diff (code and tests; the docs changes are prose and are omitted)
...
@@ -56,37 +58,32 @@ import type { PublicHead } from "../store/public-reader.js";
...
@@ -120,7 +117,11 @@ export const MANAGED_HEAD_END = "<!-- spideryarn:managed-head:end -->";
...
@@ -143,7 +144,9 @@ export function composeShell(shell: string, head: PublicHead | null): string {
...
+  return (
+    shell.slice(0, start) + tags(head, mode).join(gap) + shell.slice(end + MANAGED_HEAD_END.length)
+  );
 }
 
 /**
@@ -208,11 +211,13 @@ function requireOnce(shell: string, marker: string, at: number): void {
...
+function tags(head: PublicHead, mode: Mode): string[] {
...
@@ -221,7 +226,7 @@ function tags(head: PublicHead): string[] {
...
+  const out = [MANAGED_HEAD_START, `<title>${escapeHtml(documentTitle(head.title, mode))}</title>`];
...
@@ -266,17 +271,3 @@ function tags(head: PublicHead): string[] {
...
+import { DEFAULT_MODE, type Mode } from "../modes.js";
 ...
 /**
@@ -274,16 +275,25 @@ export async function servePublicReadPage(args: {
...
+  const body = composeShell(shell.html, decision.head, mode);
...
@@ -200,6 +201,38 @@ export function originalUrl(raw: string): string | null {
...
@@ -301,6 +334,7 @@ async function serve(req: IncomingMessage, res: ServerResponse): Promise<void> {
...
+import { APP_NAME, MODE_LABEL, SEP, TAGLINE, articleTitle, clamp } from "../title-text.js";
...
@@ -230,6 +199,93 @@ function segments(spec: TitleSpec): string[] {
   }
 }
...
+
  ...
  *
@@ -246,7 +302,7 @@ function segments(spec: TitleSpec): string[] {
...
@@ -257,36 +313,6 @@ function join(parts: string[]): string {
...
index efa319f..d07e78a 100644
--- a/src/web/params.ts
+++ b/src/web/params.ts
@@ -238,52 +238,15 @@ export const panelParam = createParser<Panel>({
...
index ae8017a..2274505 100644
--- a/tests/client-imports.test.ts
+++ b/tests/client-imports.test.ts
@@ -135,6 +135,28 @@ const SHARED = new Set([
...
@@ -340,71 +351,214 @@ describe("text from a stranger, on its way into a document head", () => {
   });
 });
...
@@ -313,6 +314,54 @@ describe("readSlug, and what a malformed capture is answered with", () => {
   });
 });
...
+ * to have open.
+ */
+export function documentTitle(title: string | null, mode: Mode = DEFAULT_MODE): string {
+  const label = mode === DEFAULT_MODE ? "" : `${MODE_LABEL[mode]}${SEP}`;
