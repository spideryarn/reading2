...
+| [`tests/glossary.test.ts`](../../tests/glossary.test.ts) | stage 5d's deterministic halves — the matching rule (including both directions in which `\b` is wrong about an accented letter), the **richness-scored dedup** that keeps the more specific phrase, the `javascript:` URL check, the occurrence pass, and `isStale` ([glossary.md](glossary.md)) |
 | [`tests/tweets.test.ts`](../../tests/tweets.test.ts) | stage 5c's deterministic halves — counting a post's characters, the artefact shape, how many posts to ask for, and **`threadIsCurrent`**, the first step freshness check in the repo ([260825g-tweet-thread-page.md](../plans/260825g-tweet-thread-page.md)) |
...
+ * separate fix. See docs/plans/260828aj-simplification-wave-2.md § 0.6.
+ *
