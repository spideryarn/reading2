...
chat pane helped anyone read better**. And [260825c-bottom-bar.md](260825c-bottom-bar.md) left it out of the bar
...
on 2026-08-25. Links followed on 2026-08-27
([260827ao-chat-web-links.md](260827ao-chat-web-links.md)), and on
...
([260825f-postgres-migration.md](260825f-postgres-migration.md)) it is one table, one row per message, and nothing
...
[260826c-half-swapped-message-ids.md](../postmortems/260826c-half-swapped-message-ids.md). The fix is one field in
...
- **A dropped SSE stream used to leave the panel on "thinking…" for ever.** Fixed 2026-08-26 —
  ...
  [260826r-sse-stall-recovery.md](260826r-sse-stall-recovery.md). In short: the server beats every 15 seconds so
...
- **A stop the server cannot honour says nothing.** `POST /stop` answers `{ stopped: false }` when
  ...
  [260825f-postgres-migration.md](260825f-postgres-migration.md) closes.
- **No keyboard shortcut** switches mode, and none opens the chat. The app has no shortcut map at
  all yet — the same gap [260825c-bottom-bar.md](260825c-bottom-bar.md#what-is-still-open) records. Escape now does
...
- **Two servers on one `data/` directory can lose a message.** The read-modify-write queue is
  ...
  away with [260825f-postgres-migration.md](260825f-postgres-migration.md). What *was* done is the cheap half: the
...
- [260828p-chat-client-architecture.md](260828p-chat-client-architecture.md) — why `useChat` grew eleven refs, what
...
- [260825c-bottom-bar.md](260825c-bottom-bar.md) — the bar the Chat button lives in, and the two kinds of button it used to have
