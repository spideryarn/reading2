...
2. **BLOCKER — Certain: Stage 5 cannot prove fresh recovery.**
   ...
   Worse, [`push-env` happens after cloud-init](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/remote-box-dev-environment.md:126), so `provision.sh` cannot clone a private repo during first boot without violating the secret boundary.
