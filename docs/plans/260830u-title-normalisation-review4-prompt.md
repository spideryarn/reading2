# Re-review: one title rule for the server and the client (round 4)

Round 3 returned **CHANGES REQUESTED** with three blockers and a nit. All four are fixed. You have
now found four of the seven divergences in this change, and two of those were on an axis I had
checked and written off in an earlier round — so the question I most want answered is still § 1 below.

## Your blocker 1 — `?slug=` and `?add=` are sixth and seventh divergences

Both fixed the way you suggested, by constraining them to the root, which is what
`docs/project/url-state.md` has always described them as:

- `main.tsx`'s `?slug=` rewrite now requires `location.pathname === "/"`.
- `canonicalAddHref` consults `addUrlFromQuery` only when `pathname === "/"`. **The path form is
  untouched** — `/add/<url>` stays canonical wherever it appears, and the existing tests for it all
  already used `/`.

**A consequence you should check, because it is the kind of thing this change keeps producing.**
`tests/router.test.ts` had a control whose whole premise was *"`?add=` is read from the query
wherever it appears, and this is the shape that shows the guard is doing work"* — it asserted that
`/auth/callback?add=…&code=SECRET` **would** be folded, to prove the `onCallback` exemption in
`main.tsx` is load-bearing. That premise is now false, and the auth callback is safe for two
independent reasons. I did not delete the case. It now asserts each guard separately, with a positive
control on `/` (the identical query, folded, still carrying the code) so that the `null` on
`/auth/callback` is evidence about the pathname rather than about a function that has quietly stopped
reading `?add=` at all — plus a static check that `main.tsx` still gates all four rewrites on
`onCallback`, since a defence that has become redundant is exactly the kind that gets deleted as dead
code. **Please check I have not weakened that control.**

## Your blocker 2 — `not-shared` treated as "ready"

Correct, and it was pre-existing rather than introduced here — the old expression mapped it to `""`
too — but it is in the function I wrote, so it is mine. The real fault is the one you named: the
hand-over was to nobody. `NotSharedPage` now sets its own title, through a new `not-shared` variant of
`TitleSpec`; the union test already enforces that every variant is constructed somewhere under
`src/web/`. It says what the page's own `<h1>` says and no more — a slug you do not own is a 404
rather than a 403, and the tab must not be the thing that confirms an article exists.
`articleWaitTitle`'s comment now states the invariant that was implicit and unchecked: **every child
it hands over to must set a title.**

## Your blocker 3 — the private rename

Accepted as an exception rather than changed, and written down as one, in
`docs/project/page-titles.md` and in a new case in `tests/public-visibility-pg.test.ts`. The
non-disclosure is asserted **first** — `head.title` is not the override, the serialised head does not
contain it, and `documentTitle(head.title)` does not contain it — then the extracted title spelled
out, then the owner's payload title, then the fact that the two differ. Mutating `loadHead` to return
the override reddens it on the first assertion with the right message.

Nobody but the owner can see the change: stranger, signed-in stranger and owner all get
byte-identical public responses (an existing case), so it is visible only to the one person who
already knows both strings.

## Your nit — `viewFor`'s documented input

Fixed. `redirectsToMetadata` and `viewFor` now document that they take **either** a query or a whole
URL, say why (the client passes `location.search`, the server passes the restored URL), and the tests
cover both shapes rather than only the documented one.

## What I want from this round

1. **An eighth.** You have found four; I have found three, and two of mine only after you pointed at
   the axis. The shape is *anything that decides what the tab says, or which page the reader ends on,
   between the first byte and React's first paint*. `main.tsx` has four rewrites (hash → `?at=`,
   `?slug=`, `?add=` via `canonicalAddHref`, and `about`/`panel`). I believe the hash one is safe
   because it changes no page and no title. Is it? And is there anything outside `main.tsx` — a
   redirect, a `<base>`, a service worker, an early `navigate()` — that I have not thought to look at?
2. **The root constraint's blast radius.** Does refusing `?slug=` and `?add=` outside `/` break any
   address a reader could plausibly hold? I believe not — `/read/a?slug=b` is a contradiction nobody
   produced — but this is a behaviour change to shipped client code and I would like it challenged.
3. **The callback control**, as above.
4. Whether this is fit to commit.

## Evidence

End to end through the **compiled** `api-dist/vercel.js` against real local Postgres, twelve
addresses; nothing in the harness imports `src/`:

`?about=1`, `?panel=about`, `?about=1&mode=glossary` → `… · Metadata · Spideryarn`.
`?about=0`, `?slug=other`, `?add=<url>`, `?mode=hierarchy`, `?mode=toc`, `?mode=%zz%zz`, bare
→ `… · Spideryarn`. `?mode=glossary` → `… · Glossary · Spideryarn`. Unknown slug → 404, `Spideryarn`,
no `og:` tags. `og:title` and `og:url` carry neither mode nor view in any row.

Mutations added this round, each red on the assertion named for it: `loadHead` returning the private
override (dies on the non-disclosure assertion first); and the four from round 3 re-run.

`npm run typecheck`: clean on the two projects that are mine; the tests project has 2 errors, both in
untracked peer files (`tests/profile-panel.test.tsx`, `tests/toc-structure-request-parity.test.ts`).
Both builds pass. Shell self-test 46/46. Biome clean on every file in this diff. Full suite: 6
failures — `auth-callback.test.ts` (verified failing at HEAD without this change),
`doc-links.test.ts` (a peer's untracked `src/sketch.ts` pointing at a plan doc that does not exist),
`toc-write-guard.test.ts` (fails alone; `src/toc.ts` is a peer's uncommitted edit), and pg suites
that pass in smaller groups.
...
@@ -195,7 +196,15 @@ if (!onCallback && isSpideryarnId(legacyAnchor)) {
...
+/* **On the root and nowhere else.** The old address was `/?slug=…`, from when
+   the slug was a parameter on the one page there was — never `/read/a?slug=b`,
+   which is a contradiction nobody ever produced. Unconstrained it fired there
+   anyway, and that is a divergence rather than dead code: `/read/a` with a
+   query is one path segment, so the server composes *article a's* title, and
+   the client then rewrites the address to article b. Seventh of these; GPT Sol
+   found it and the one below on 2026-08-30, after two rounds in which I had
+   said the axis was covered. docs/project/url-state.md. */
+if (!onCallback && legacySlug && location.pathname === "/") {
   const rest = location.search
     .replace(/^\?/, "")
     .split("&")
@@ -238,8 +247,12 @@ if (!onCallback && aboutish) {
     .split("&")
...
@@ -455,7 +460,15 @@ export function canonicalAddHref(pathname: string, search: string, hash: string)
...
+import { canonicalAddHref, parseRoute } from "../src/web/router.js";
 
 const ROOT = path.resolve(import.meta.dirname, "..");
 
@@ -313,6 +316,226 @@ describe("readSlug, and what a malformed capture is answered with", () => {
   });
 });
...
+    /* `main.tsx` gates its rewrite on `location.pathname === "/"`. Asserted as
+       text because the rewrite is a module-init side effect nothing can call —
+       static, and it says so; the behavioural half is the `?add=` case below,
+       which is a pure function. */
+    const main = readFileSync(
+      path.join(path.resolve(import.meta.dirname, ".."), "src", "web", "main.tsx"),
+      "utf8",
+    );
+    expect(main).toContain('legacySlug && location.pathname === "/"');
+  });
+
...
+
 ...
 import {
   ADMIN_HREF,
@@ -548,14 +551,53 @@ describe("the auth callback", () => {
...
+
+  /**
+   * Guard two is a side effect of loading main.tsx, which nothing can call —
+   * so it is asserted as text. Static, and it says so: it proves the exemption
+   * is written, not that it runs. It exists because the guard above now makes
+   * the callback safe without it, and a defence that has become redundant is
+   * exactly the kind that gets deleted as dead code.
+   */
+  it("and main.tsx still exempts it, which no unit test can otherwise see", () => {
+    const main = readFileSync(
+      path.join(path.resolve(import.meta.dirname, ".."), "src", "web", "main.tsx"),
+      "utf8",
+    );
+    expect(main).toContain("onCallback");
+    /* Every rewrite in that file is gated on it. Counted rather than merely
+       present, so deleting one gate reddens this. */
+    expect(main.match(/!onCallback/g) ?? []).toHaveLength(4);
   });
 });
