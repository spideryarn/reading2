...
The lock is released before Storage deletion (plan (`docs/plans/260827o-raw-bytes-in-storage.md`), lines 226–235).
...
The plan says fetch uploads and then “registers” the object (plan (`docs/plans/260827o-raw-bytes-in-storage.md`), lines 307–308).
...
The plan retains filesystem blobs for “a laptop with no container” (plan (`docs/plans/260827o-raw-bytes-in-storage.md`), lines 301–303). In that configuration there is no Postgres `raw_sources` row to lock.
