...
The plan captures a SHA, then later runs plain `git push origin main` ([plan lines 197–203](</Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/deploy-pipeline.md:197>), [249–260](</Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/deploy-pipeline.md:249>)). Another agent can commit during the 30-second test run or migration. The push then ships the new `main`, while the gates, migrations, and polling still refer to the old SHA.
...
only exports the variable to `npm run build`. The API build after `&&` does not inherit it ([plan lines 364–375](</Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/deploy-pipeline.md:364>)).
...
The symlinks are the problem ([plan lines 205–220](</Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/deploy-pipeline.md:205>)):
...
Printing the rollback warning after failure is too late ([plan lines 310–312](</Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/deploy-pipeline.md:310>)). Check auto-assignment before migrations. If it is off, abort before touching the database.
