...
- `articleId` is required by both composite block foreign keys; see `docs/plans/delete-the-importer.md:253-255`.
...
The plan’s phrase “`has()` must consult `revision_step_runs` and compare the stamp” is too broad (`docs/plans/delete-the-importer.md:256`). `has` receives no expected stamp, and expected input is step-specific: `ideas` hashes blocks plus tree, while other steps use different inputs (`src/pipeline.ts:1338-1349`). Teaching the adapter those rules would duplicate pipeline logic in storage.
...
That recreates the exact class documented in `docs/postmortems/toc-status-never-checked.md:89-106`.
