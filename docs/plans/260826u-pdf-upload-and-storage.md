...
This is **step 7 of [260826c-pdf-ingestion.md § Build order](260826c-pdf-ingestion.md#build-order)** — *"Upload
...
[260826c-pdf-ingestion.md § Upload](260826c-pdf-ingestion.md#upload) still stands; this file is that section
worked out to the point where somebody could build it, plus the object store it needs, which is
new. It is a separate file only because `260826c-pdf-ingestion.md` is already sixteen hundred lines and
...
>
> — Greg, 2026-08-26, [260826c-pdf-ingestion.md § Greg's answers](260826c-pdf-ingestion.md#gregs-answers-2026-08-26)
...
exactly the trap `260826c-pdf-ingestion.md` warns about: *"Building the local one first is fine; mistaking
...
`260826c-pdf-ingestion.md`'s "local dev only" multipart sketch, and the reason is that plan's own warning:
...
`260826c-pdf-ingestion.md` states the rule and it is the load-bearing sentence of this whole design:
...
`260826c-pdf-ingestion.md` already lists it and it is worth restating because it is easy to under-estimate
from the diagram:
...
- **The slug cycle.** Already decided in `260826c-pdf-ingestion.md` and carried here unchanged: store under
...
1. **Where do the bytes finally live?** The one thing `260826c-pdf-ingestion.md` left open and marked
...
[260825f-postgres-migration.md](260825f-postgres-migration.md)'s schema — and
[`src/db/schema.ts`](../../src/db/schema.ts) as built — puts the fetched bytes in a `raw_bytes`
`bytea` column on `article_revisions`. `260826c-pdf-ingestion.md` says an uploaded PDF's bytes live in a
...
- **`260826e-postgres-storage-implementation.md` already flags the cost**: `raw_bytes` at the fetch ceiling
...
> without: [260827o-raw-bytes-in-storage.md](260827o-raw-bytes-in-storage.md). Two things there are worth reading back
...
follow-on rather than a rewrite, which is why `260826c-pdf-ingestion.md` was right to insist on the seam
before the feature.
...
[260826z-pdf-upload-storage-review-sol.md](260826z-pdf-upload-storage-review-sol.md). Every code reference below was
...
Full text: [260826ag-pdf-upload-code-review-sol.md](260826ag-pdf-upload-code-review-sol.md); the prompt is
[260826ai-pdf-upload-code-review-prompt.md](260826ai-pdf-upload-code-review-prompt.md). [AGENTS.md](../../AGENTS.md)
...
4. **The four upload refusals were invisible to the message registry.** `kindOfMessage` returns
   ...
   a Retry button that could not work — [toc-max-tokens](../postmortems/260826a-toc-max-tokens.md) again.
...
```ts
     ...
     * is the failure docs/plans/260826u-pdf-upload-and-storage.md § Appendix exists to
     * avoid.
     */
    rawSourceId: uuid("raw_source_id").references(() => rawSources.id),
```
...
([260826q-job-queue-rethink.md](260826q-job-queue-rethink.md)). An upload record is queue state: created, claimed
...
out of the same staging key, for ever. That is [toc-max-tokens](../postmortems/260826a-toc-max-tokens.md)
...
[260827c-pdf-upload-code-review-sol-2.md](260827c-pdf-upload-code-review-sol-2.md); the prompt is
[260827b-pdf-upload-code-review-2-prompt.md](260827b-pdf-upload-code-review-2-prompt.md). Its verdict was
...
arrives with [260826q-job-queue-rethink.md](260826q-job-queue-rethink.md), which is where the `uploads` table
belongs.
...
- [260826z-pdf-upload-storage-review-sol.md](260826z-pdf-upload-storage-review-sol.md) — the cross-family review in
  full, and [260826v-pdf-upload-storage-review-prompt.md](260826v-pdf-upload-storage-review-prompt.md), the prompt
- [260826c-pdf-ingestion.md](260826c-pdf-ingestion.md) — the plan this is step 7 of; § Upload is its short form
...
- [260826e-postgres-storage-implementation.md](260826e-postgres-storage-implementation.md) — the `raw_bytes` cost
