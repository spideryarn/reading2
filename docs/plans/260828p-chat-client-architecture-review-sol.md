...
3. The phase collapse is sound, but its safety claim is false. A stale admitted failure could still change `"ready"` to `"failed"`. The `live` gate prevents bug 10; `phase` merely removes `(loaded=false, loadFailed=true)` ([useChat.ts](/Users/greg/Dropbox/dev/experim/spideryarn2/src/web/useChat.ts:586), [plan](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/260828p-chat-client-architecture.md:95)).

4. The plan’s count of three `setComments` sites is wrong; there are ten ([plan](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/260828p-chat-client-architecture.md:30)).
