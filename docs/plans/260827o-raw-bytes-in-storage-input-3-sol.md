...
1. **Critical — mismatched Supabase projects creates the requested dangling reference without deletion. Confidence: 100%.**
   ...
   `src/store/blobs.ts:108-117` and `src/db/client.ts` select these independently. The third draft notices this at `docs/plans/260827o-raw-bytes-in-storage.md:320-339`, but the reduced protocol must retain a startup check that proves project identity—not merely “Postgres plus Supabase Storage are configured.” For hosted Supabase, compare the project ref in `SUPABASE_URL` with the ref encoded in the database username.
