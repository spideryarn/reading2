...
The plan says `POST /api/jobs` verifies before enqueue (`docs/plans/260826u-pdf-upload-and-storage.md:215-234`), then says byte-dependent checks happen in the worker (`docs/plans/260826u-pdf-upload-and-storage.md:236-237`).
...
`signUpload` does not belong beside ordinary server-side blob operations. The filesystem implementation cannot implement it, and a health check cannot repair an interface that lies (`docs/plans/260826u-pdf-upload-and-storage.md:173-200`).
