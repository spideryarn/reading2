...
6. LOW — The claimed LRU is FIFO.
   ...
   Evidence: a hit at `src/similar.ts:165-167` does not refresh insertion order, while eviction deletes the first inserted entry at `src/similar.ts:295-300`. The plan explicitly calls it an LRU at `docs/plans/force-diagram-links.md:193-197`.
...
9. LOW — Passage evidence is computed but never shown.
   ...
   Evidence: semantic edges retain passage IDs at `src/web/graph.ts:789-840`, and the plan says the card names those passages at `docs/plans/force-diagram-links.md:175-178`. `src/web/DiagramPanel.tsx:460-469` discards `passages`; the card shows only section title and cosine.
...
10. LOW — Several comments still describe a different protocol or marker configuration.
   ...
   Evidence: the plan says GET at `docs/plans/force-diagram-links.md:153-155`; so do `src/web/useSimilar.ts:9`, `src/web/graph.ts:307` and `src/types.ts:864`, while the implementation is POST at `src/routes.ts:2692`. `src/web/DiagramPanel.tsx:673-676` says `markerUnits="strokeWidth"` while the marker uses `userSpaceOnUse`.
