...
A sixth [mode](chat-mode.md) in the band, beside [Glossary](../project/glossary.md). The glossary
...
[one mode owns the band at a time](chat-mode.md), and leaving a mode takes its marks with it — but
...
What it *does* reuse is everything that made the glossary cheap: the [mode band](chat-mode.md) slot
...
[chat-mode.md](chat-mode.md#say-the-awkward-thing-first) set the precedent that it goes at the top of
...
- **It does not stream.** It goes through the job queue with `JobProgress`, exactly as glossary and
  ...
  existing one: [streaming-the-slow-two.md](streaming-the-slow-two.md) is the plan for that class, and
...
- [search.md](../project/search.md) — the marks, the fallback, the rail, and the ruler that would have
  been silently wrong
- [chat-mode.md](chat-mode.md) — the mode band this reuses, and the reframing that made it a slot
...
- [streaming-the-slow-two.md](streaming-the-slow-two.md) — where the "it does not stream" debt is
  tracked
