...
The plan and its two earlier reviews (yours) are `docs/plans/260826u-pdf-upload-and-storage.md` — read it,
...
The PDF extraction pipeline it feeds is `docs/plans/260826c-pdf-ingestion.md`; the ingest queue is
...
1. **No Postgres `uploads` / `raw_sources` tables.** The plan specifies them. I put the upload
   ...
   `src/db/schema.ts` is unused pending `docs/plans/260826q-job-queue-rethink.md`. The state machine is in
...
- **The client.** `src/web/upload.ts` uses `XMLHttpRequest` for upload progress. Abort handling,
  ...
  mid-upload.
- **Anything in `docs/plans/260826u-pdf-upload-and-storage.md` that the code now contradicts**, since the
