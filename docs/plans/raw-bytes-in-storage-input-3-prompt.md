...
You have reviewed `docs/plans/raw-bytes-in-storage.md` twice, both NO-SHIP, both times on the
...
2. **One transaction**: upsert the `raw_sources` row (`on conflict (sha256, kind) do nothing`), write
   ...
   This is the same single fenced commit `docs/plans/transactional-stage-runner.md` already requires.
...
1. `docs/plans/raw-bytes-in-storage.md` — the third draft, whose deletion protocol is what is being
   removed.
2. `docs/plans/raw-bytes-in-storage-review-2-sol.md` — your own second review.
