...
**Declared in `src/db/auth-users.ts`** — since deleted, when the accounts moved to the Auth service's Admin API ([260828f-admin-id-was-the-local-one.md](../postmortems/260828f-admin-id-was-the-local-one.md)) — **deliberately *not* in
...
[260825f-postgres-migration.md](260825f-postgres-migration.md) arrives it will need `usage` on schema `auth` and
...
The code went back to Sol (`docs/plans/260827z-admin-page-code-review-sol.md`) and came back **no blocker,
