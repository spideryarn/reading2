...
**Status: built, 2026-08-27.** Builds on [diagram-mode.md](diagram-mode.md), which built the six
pictures, and on [semantic-search.md](semantic-search.md), which chose the embedding model and has
still not been built.

**GPT-5.6 Sol reviewed this plan before it was built and returned NO-SHIP** —
[the review](force-diagram-links-review-sol.md), ten findings, six of them blockers. All ten were
...
account. See [semantic-search.md](semantic-search.md) § The model. Nothing here re-litigates that.
...
[semantic-search.md](semantic-search.md) — it needs pgvector, a migration, a re-embed-on-change rule
...
2. **Is a memory-only cache good enough**, or should this wait for the pgvector work in
   semantic-search.md? Recommendation: memory, on the grounds above.
