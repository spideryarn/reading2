...
The same modelling gap exists for load: the proposed gate looks only in `state.operations`, while load’s `opId` lives separately ([plan](</Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/260828v-chat-operation-model.md:30>)). Define registration/input events separately from gated asynchronous-result events.
