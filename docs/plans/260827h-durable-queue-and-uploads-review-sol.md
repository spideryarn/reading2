...
- The superseded migration plan correctly required the shared transactional stage runner before the advance endpoint: `260826e-postgres-storage-implementation.md:118`
...
- The plan relies on two independent fences: `260827h-durable-queue-and-uploads.md:199`
...
- The rethink explicitly requires draft, step-run, publication and job transition in one transaction: `260826q-job-queue-rethink.md:242`
...
- Neither the jobs schema nor the proposed additions contain an active work key or slug reservation: `src/db/schema.ts:577`, `260827h-durable-queue-and-uploads.md:304`
...
Claim changes a row to `running`, but the plan only explicitly clears the attempt and lease for terminal completion: `260827h-durable-queue-and-uploads.md:214`.
...
The proposed local loop exits when it sees `busy`: `260827h-durable-queue-and-uploads.md:360`.
...
The upload’s conditional `pending → claimed` update is a correct replacement for the filesystem `wx` marker. However, later `verified`/`rejected` transitions are fenced only by upload status, not the owning job attempt: `260827h-durable-queue-and-uploads.md:292`.
