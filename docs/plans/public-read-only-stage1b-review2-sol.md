...
3. **blocker — the settled `Referrer-Policy` requirement is not implemented.** The plan requires an explicit policy in [public-read-only-access.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/public-read-only-access.md:820), but [vercel.json](/Users/greg/Dropbox/dev/experim/spideryarn2/vercel.json:16) sets only `X-Robots-Tag`. Article-supplied outbound links and subresources do not all carry their own policy.  
