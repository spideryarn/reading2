...
two plans it re-sequences — `docs/plans/transactional-stage-runner.md` (you returned NO-SHIP on its
first draft; the corrections are folded in) and `docs/plans/raw-bytes-in-storage.md` (three rounds,
...
1. `docs/plans/delete-the-importer.md` — the new plan. The subject of this review.
2. `docs/plans/transactional-stage-runner.md` — the landings it re-sequences. Especially
   § The order (A–E), § What has to be true, and § Open.
3. `docs/plans/raw-bytes-in-storage.md` — § The part the first draft did not have, § The backfill can
