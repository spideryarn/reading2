...
[260831b-finish-the-database-move.md § Stage 2](260831b-finish-the-database-move.md) and
[260827aa-delete-the-importer.md § D2 scoped](260827aa-delete-the-importer.md).
...
`260827aa-delete-the-importer.md` § *D2 scoped* says the sweep *"sweeps the `data/` root directly, so it is a
...
- **Correctness: unaffected.** Everything the model saw is in `batchFingerprint` — `version`,
  ...
  changed key. `260827aa-delete-the-importer.md` § *D2 scoped* already argued this when it retired the
  `sourceHash` gate.
- **Cost: a saving.**
...
- `260827aa-delete-the-importer.md` § *D2 scoped* — the sweep does carry over; fix the stale line numbers to
  `src/toc.ts:1384` / `src/labels.ts:2544`, or better, stop quoting line numbers for this call.
- `260831b-finish-the-database-move.md` § Stage 2 — replace *"Unresolved, settle before building"* with the
