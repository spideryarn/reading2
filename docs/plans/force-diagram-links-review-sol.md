...
4. **HIGH — Anchor edges need aggregation and a density bound.**
   ...
   - **Evidence:** The plan only drops same-node links (`docs/plans/force-diagram-links.md:94`). Existing resolution explicitly treats inline footnote targets as ordinary internal destinations (`src/web/internal-links.ts:28`).
...
5. **BLOCKER — Per-block top-K cannot reliably produce the client’s global top handful.**
   ...
   - **Evidence:** The server returns each block’s top neighbours, while the client subsequently collapses, globally sorts, and truncates (`docs/plans/force-diagram-links.md:105`, `docs/plans/force-diagram-links.md:119`).
...
6. **BLOCKER — `|i-j| ≤ 1` does not remove reading-order duplication.**
   ...
   - **Evidence:** The plan excludes only adjacent block indices (`docs/plans/force-diagram-links.md:115`), while the proposed chain operates over deepest drawn nodes (`docs/plans/force-diagram-links.md:33`).
...
7. **BLOCKER — The proposed extraction copies an eval client that is not production-safe.**
   ...
   - **Evidence:** Current validation stops at length and missing entries (`evals/embedding-retrieval.ts:418`). The semantic-search plan already records the additional mandatory production checks (`docs/plans/semantic-search.md:238`). The router exposes thrown messages in its JSON 500 response (`src/routes.ts:2848`).
...
8. **BLOCKER — The paid GET and memory cache are neither bounded nor dependable.**
   ...
   - **Evidence:** The route is a paid `GET` (`docs/plans/force-diagram-links.md:105`); the plan calls the cost “bounded, cached” (`docs/plans/force-diagram-links.md:175`). `useIdeas` explicitly separates its GET read from the job that performs the model call (`src/web/useIdeas.ts:1`). Fluid Compute serves concurrent requests in one instance (`docs/project/logging.md:689`), while process-local state cannot guarantee reuse across instances.
...
9. **BLOCKER — The plan never specifies how late semantic edges change the physics.**
   ...
   - **Evidence:** The plan says the edges arrive after the initial picture (`docs/plans/force-diagram-links.md:124`). The graph and layout are currently synchronous memos (`src/web/DiagramPanel.tsx:251`); `layoutForce` constructs its simulation exclusively from `graph.edges` (`src/web/diagram-d3.ts:222`). `DiagramPanel` does not even receive a slug yet (`src/web/DiagramPanel.tsx:69`).
...
10. **HIGH — Optional `DiagramLink.kind` leaves the exact compiler trap this refactor should remove.**
   ...
   - **Evidence:** `DiagramLink.depth` is currently required (`src/web/diagram.ts:185`); Arc assigns it from similarity weight (`src/web/diagram-d3.ts:400`); the panel emits only the depth class (`src/web/DiagramPanel.tsx:489`). The plan proposes an optional discriminator (`docs/plans/force-diagram-links.md:140`).
