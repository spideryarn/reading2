...
The prompt says the article’s words “settle it” and Socratic should point to the passage that “would change their mind” ([review-mode.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/review-mode.md:284)). That assumes the model’s interpretation is ground truth.
...
Also, `"just tell me"` currently cannot work: Socratic promises it as an exit, but the next turn’s unchanged Socratic picker still “governs the whole reply” ([review-mode.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/review-mode.md:293)). Explicit words from the reader must override the picker, or “just tell me” must be a real UI action that selects Respond.
...
- “two or three points”
- “I think you’ve got this” ([review-mode.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/review-mode.md:228))
...
- The check must happen before `settleThread`; otherwise it recreates the exact bug where a rejected retry aborts another tab’s live answer ([chat-mode.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/chat-mode.md:864)).
...
Do not make `ReviewBand` a near-copy with a second `useChat`. Use one conversation-band wrapper, mounted for both Chat and Review, with one hook and mode-specific panel/footer. The current unmount/remount path already has a known race that can hide a just-started conversation ([chat-mode.md](/Users/greg/Dropbox/dev/experim/spideryarn2/docs/plans/chat-mode.md:1019)).
