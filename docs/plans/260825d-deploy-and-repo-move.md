...
3. ~~**The queue, honestly.** `POST /api/jobs` returns its 202 receipt and continues the work with
   ...
   **And `waitUntil` would not have rescued it anyway.** It is best-effort: no retries, no durability,
   ...
   bullet used to point at** — see [260826q-job-queue-rethink.md](260826q-job-queue-rethink.md), which chose a
...
[auth.md](../project/auth.md), and [260825a-auth-options.md](../research/260825a-auth-options.md) for the survey
...
- [auth.md](../project/auth.md) — the gate's own doc, and
  [260825a-auth-options.md](../research/260825a-auth-options.md) for why the provider is Supabase Auth
