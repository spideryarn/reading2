# Re-review: one title rule for the server and the client (round 5)

Your round-4 finding was the eighth divergence, and it changed how this is being fixed. Greg's
instruction on seeing the count was: *"might as well fix them. Hopefully we're fixing them in a
clean, robust, general way."* He is right, and your finding is the proof — you wrote that **the
separate predicate tests cannot catch this interaction**, and eight bugs fixed one at a time with one
test each is exactly the shape of testing that let each next one through.

So this round is structural rather than another patch.

## Your blocker — the hash rewrite reserialising the query

Fixed, and the fix is a rule rather than a branch. `main.tsx`'s own comments already said the query
must be edited as **text** and never round-tripped, because `URLSearchParams` re-encodes `?cols=0,1`
into `?cols=0%2C1`. Two of the four rewrites obeyed that; the hash one used `new URL()` +
`searchParams.set()`, which is what normalised `%61bout=1` into `about=1`. It now edits text like the
others, so `%61bout`, `about=%31` and `panel=%61bout` all survive — and `?cols=0,1` stops being
mangled, which was a live cosmetic bug nobody had noticed.

## The structural change

**`main.tsx`'s four pre-mount rewrites are now one pure function**, `settleAddress(pathname, search,
hash)` in `router.ts`, composing them in the same order. `main.tsx` calls it once. Three consequences:

1. **Interactions are testable.** The sequence had no tests at all; each rewrite had its own. That is
   precisely where your eighth lived.
2. **One `onCallback` guard instead of four.** A fifth rewrite added inside `settleAddress` is exempt
   from the auth callback by construction — which answers your round-4 note that counting four
   `!onCallback` tokens would not catch an unguarded fifth. The router test now counts *rewrite
   sites* (exactly one `history.replaceState` in `main.tsx`) and adds a behavioural half: an
   unguarded `settleAddress` really would rewrite a callback address carrying a `code`.
3. **The three static grep-for-a-string tests I had written became behavioural**, because the thing
   they were asserting about is now callable.

## The general guard

`tests/address-settling.test.ts` states the class instead of the cases. It crosses every path shape
against every query parameter this app has ever recognised (21, including the encoded spellings your
finding turned on) against every hash shape (4), and for each address asserts

    what the server puts in <title>  ===  what the client ends up setting

It compares the **real** functions on both sides — `readSlug`, `readMode`, `viewFor`, `composeShell`
against `settleAddress`, `parseRoute`, `pageTitle` — so it is two behaviours rather than two models.
The `client === null` branch (the reader leaves this article) is checked too, which is what covers
`?slug=` and `?add=`.

Restoring three of the eight faults reddens it, verified: the transport dropping the mode; the server
not predicting the metadata rewrite; and the hash rewrite reserialising the query, which fails with
three encoded spellings printed server-title against client-title. It carries a control requiring
more than forty addresses to actually reach the reading view, since a cross-product that compares
nothing also reports no disagreement.

## What I want from this round

1. **Is the cross-product the right shape, and is it complete along the axes that matter?** It varies
   query and hash over one path. Should it vary the path too — `/read/x` vs `/` vs `/add/…` vs
   `/read/x/metadata` — and would that catch anything the per-case tests do not?
2. **`settleAddress` is a behavioural rewrite of the app's entry point.** The order is preserved and
   `router.test.ts` passes unchanged, but this is module-init code whose bugs the file's own comments
   say were "only findable in a browser". What did I get wrong in the composition — the `Address`
   splitting, the `hash` carried through `liftLegacySlug` and `liftLegacyAbout`, the `?at=` ordering
   difference between `searchParams.set` (replaces in place) and my append?
3. **A ninth.** With the sequence now callable, is there an address where `settleAddress` and the
   server still part company that the corpus does not contain?
...
on every file in this diff. The eleven test files this change touches: 251 passed, 0 failed.

Two things I cannot show you this round, stated rather than glossed:

- **The end-to-end run through the compiled bundle is unavailable.** A peer's test run has emptied
  almost every article's `current_revision_id` in the shared local database, including the one public
  article, so every address now 404s correctly for want of artefacts. The twelve-address table from
  round 4 was taken when the data existed and is unchanged by this round's edits; I am not re-running
  it and not claiming I did.
- **The full suite is unusable right now**: 68 failures, with two wedged `queued`/`running` rows
  holding the single job slot. Same condition as the 99-failure run I reported earlier, which cleared
  on its own. The eleven files above pass.
...
@@ -455,12 +461,161 @@ export function canonicalAddHref(pathname: string, search: string, hash: string)
...
+  const rest = withoutPairs(at.search, (pair) => /^at=/.test(pair));
...
+  if (!slug) return at;
+  const rest = withoutPairs(at.search, (pair) => pair.startsWith("slug="));
...
+  settleAddress,
   ...
   ADMIN_USERS_HREF,
   addHref,
@@ -548,14 +552,74 @@ describe("the auth callback", () => {
...
+
+import { DEFAULT_MODE, MODES } from "../src/modes.js";
+import { composeShell } from "../src/public/page-head.js";
+import { viewFor } from "../src/read-address.js";
+import type { PublicHead } from "../src/store/public-reader.js";
+import { readMode, readSlug } from "../src/vercel.js";
+import { pageTitle } from "../src/web/page-title.js";
...
+
+/**
+ * **What the server will put in the tab for this address.**
+ *
+ * Through the real transport functions — `readSlug`, `readMode`, `viewFor` —
+ * and the real composer, so that a change to any of them shows up here. Returns
+ * `null` when the address is not one the function composes a head for.
+ */
+function serverTitle(pathname: string, search: string): string | null {
+  const url = `${pathname}${search}`;
+  const slug = readSlug(pathname);
+  if (slug !== SLUG) return null;
+  const composed = composeShell(SHELL, HEAD, readMode(url), viewFor(url));
+  return /<title>([\s\S]*?)<\/title>/.exec(composed)?.[1] ?? null;
+}
+
+/**
...
+  it("agree for every combination of query and hash on a shared article", () => {
+    const disagreed: string[] = [];
+    let compared = 0;
+
+    for (const search of QUERIES) {
+      for (const hash of HASHES) {
+        const server = serverTitle(`/read/${SLUG}`, search);
+        const client = clientTitle(`/read/${SLUG}`, search, hash);
+
+        /* The client leaving this article entirely is not a disagreement about
+           a title — it is the case where the server must not have claimed one.
+           Both halves are checked: `null` on the client demands `null` or a
+           default-headed response from the server, which is what `viewFor`
+           and the root constraints deliver. */
+        if (client === null) {
+          if (server !== null && server !== `${TITLE} · Spideryarn`) {
+            disagreed.push(`${search}${hash}: client left, server said ${JSON.stringify(server)}`);
+          }
+          continue;
+        }
+        compared++;
