...
7. Several claims should be softened or corrected:
   ...
   - “No app code is wrong” and “all proved in a browser” are overclaims: a real production Google callback, automatic identity link, owner UUID preservation, and shelf read have not yet been proved ([plan:164](../../docs/plans/google-sign-in-production.md)).
   ...
   - The older auth plan claims `aud` and `iss` are checked ([auth-supabase.md:494](../../docs/plans/auth-supabase.md)); current `requireUser` checks neither ([auth.ts:262](../../src/auth.ts)). That will not break this sign-in, but the security claim is false.
