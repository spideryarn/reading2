...
**The plan is `docs/plans/defer-arc-and-rename-hierarchy.md`.** Read it first, in full.
...
6. Once the arc is generated in the background, the client has to read it. There is no
   ...
   `docs/plans/glossary-read-latency.md` records a very similar refetch being expensive on the
