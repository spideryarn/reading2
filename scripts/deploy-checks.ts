 ...
 * reason: it also holds years of one-off script scratch files
 * (`output/*.mts`, `output/msg*.txt`, …), so the bare directory outlives any
 * one article's files too.
 *
 * The files named below are the ones tests actually open by path, not a
 * superset: `tests/artefact-copy.test.ts`'s `OWNED` list is the artefact
 * store's own account of what it owns for `data/writes`, and
