 ...
 *
 * Two callers, and the second generalised it. The bar's **experimental switch**
 * (Dock.tsx § the switch itself), where it is never the only carrier — the
 * button draws a warning marker, and the same sentence is in an `sr-only` span
 * it points `aria-describedby` at. And, from 2026-09-07, the bar's **mode
 * buttons**, where it holds the sentence a *visitor* gets about a mode they
 * cannot have (`markedModes`, visitor.ts). That one is not mid-flight and is
 * not broken, which is the widening: what the two share is that a reader
 * opening this card wants to know why the control looks the way it does before
 * they want to know what it is for.
 */
