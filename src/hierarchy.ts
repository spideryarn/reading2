...
async function main(): Promise<void> {
  ...
  /* **Only where a wave was asked for**, because a line about a feature that is
     off would be noise on every run — and, printed at zero, would read as though
     the deepening had been tried and found nothing. `run.deepen` is `null`
     exactly when nobody asked (src/hierarchy-deepen.ts § `DEEPEN_ENV`), and
     `deepenFailed` is the case that must not look like the absence. */
  if (run.deepen !== null || run.deepenFailed) {
    const d = run.deepen;
    console.log(
      run.deepenFailed || d === null
        ? `Deepened:  FAILED — the wave threw; this is the tree wave 1 produced`
        : `Deepened:  ${d.expanded} of ${d.targets} section(s) into ${d.added} node(s), ` +
            `${d.calls} call(s) (${d.resumed} resumed), ` +
            `${d.verdicts.rawYes}/${d.verdicts.rawYes + d.verdicts.rawNo} children want another level` +
            (d.oversized + d.outOfTime + d.rateLimited > 0
              ? `; ${d.oversized} too large, ${d.outOfTime} out of time, ${d.rateLimited} rate limited`
              : "") +
            /* **Only when there is something to say**, and then both halves of
               it: what the next attempt gets free, and what it will have to buy
               again because the row never landed. */
            (d.withheld + d.uncheckpointed > 0
              ? `; ${d.withheld} withheld with rows, ${d.uncheckpointed} withheld with none`
              : ""),
    );
  }
