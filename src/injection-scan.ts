...
/**
 * **Every shape in this file's answer lives in
 * [`src/injection-scan-types.ts`](injection-scan-types.ts)**, and is re-exported
 * here so that nothing which imported it from this module had to change.
 *
 * It moved on 2026-09-01, when the Referee band got a panel over this scan. The
 * rule is tests/client-imports.test.ts's: nothing under `src/web/` may name a
 * module that reaches a `node:` import or a heavy dependency, and this one
 * parses documents with jsdom. The fix that file's own header prescribes is to
 * *move the shared thing into a module that imports nothing*, which is what
 * `referee-mirror-types.ts` did one feature earlier.
 *
 * **Only the declarations moved.** No detection rule, no threshold and no
 * regular expression in this file changed with them.
 */
export type {
  BlindSpot,
  FindingKind,
