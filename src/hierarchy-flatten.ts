 ...
 *
 *   npm run hierarchy:flatten -- example/tree.json [maxDepth]
 *
 ...
 */
export function flattenTree(tree: Tree, options: { maxDepth?: number } = {}): HierarchyRow[] {
...
async function main(): Promise<void> {
  ...
  if (!treePath) {
    console.error("Usage: tsx src/hierarchy-flatten.ts <tree.json> [maxDepth]");
    process.exit(1);
  }
