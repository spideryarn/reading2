 ...
 *
 * `STEPS.labels.stamp` (src/pipeline.ts) declares the prompt version it would
 * write if it ran right now, and `stampOf` reads the same string back off the
...
/**
 * **What every labels manifest says, whether or not a label run has happened.**
 *
 * The three hashes and the slug are facts about the *tree and the blocks* the
 * manifest is beside, and stage 4 knows all four before it asks a model
 * anything. That is why they are here and `version`/`generator` are not: those
 * two are facts about a call, and a manifest written before any call was made
 * has no honest value for them. See `PendingLabelsFile`.
 */
interface LabelsManifest {
  slug: string;
  /**
  ...
  /**
   * The label per block id — **empty on a `PendingLabelsFile`**, and a reader
   * that needs to tell an unbought manifest from a genuinely label-less article
   * must ask `batches === null` rather than counting the keys.
   */
...
/**
 * **A manifest stage 4 writes before anybody has asked a model for a label** —
 * the tree is cut, the hashes are known, and the labels have not been bought.
 *
 * `batches: null` is the discriminant, and it is not a new convention: this
 * file's own note on `CompletedLabelsFile.batches` has said since 2026-08-26
 * that `null` is *no evidence a run happened* as against `[]`'s *evidence of
 * none*. Reusing it is what lets a reader narrow with `batches === null`
 * instead of guessing from an empty `labels` map — which a real run over an
 * article of nothing but captions could also produce.
 *
 * **No `version`, no `generator`, and their absence is the honest part.** No
 * labels prompt ran and no model answered, so recording either would invent
 * provenance — and it would do so in the two fields `stampOf`
 * (src/store/artifacts.ts) reads as `promptVersion` and `model`, which is how a
 * `revision_step_runs` row carried forward by `beginDraftIn` could be made to
 * look current against a manifest holding no labels at all. They are `?: never`
 * rather than merely missing so the compiler refuses the mistake at the literal
 * (tests/labels-file-union.test.ts).
 *
 * What makes the empty manifest safe is not this type: it is `writeArtefacts`
 * (src/store/artifacts-pg.ts), which reads `batches === null` and deletes the
 * revision's `labels` receipt in the same transaction.
 * docs/plans/260906a-labels-leave-the-blocking-hierarchy-step.md § Fable's
 * arbitration.
 */
export interface PendingLabelsFile extends LabelsManifest {
  batches: null;
  version?: never;
  generator?: never;
}

/** A manifest a real label run produced: every batch it made, and by what. */
export interface CompletedLabelsFile extends LabelsManifest {
  /** The prompt that wrote them, e.g. `labels/2`. Read by `stampOf` as `promptVersion`. */
  version: string;
  /** The model that ran. Read by `stampOf` as `model`. */
  generator: string;
  /**
   * The calls that produced these labels — **never null on this shape**, which
   * is the whole of the discriminant.
   *
   * `null` means *no evidence a run happened* as against `[]`'s *evidence of
   * none*, and that distinction is older than the union: `example/labels.json`
   * was rebuilt from the labels already sitting in a tree written before this
   * stage existed, and an empty array there was a lie to the type — `batches:
   * []` says "generated by zero calls", a fact about a run that happened. The
   * eval read it as a batched file and reported "0 batched calls".
   * GPT-5.6-sol, 2026-08-26. A synthetic batch would have been the worse fix,
   * because it would invent seam boundaries and cost figures nothing measured.
   *
   * **What the union changed, and the one committed file it leaves outside.**
   * Since 2026-09-06 `null` is `PendingLabelsFile`, which also has no `version`
...
/**
 * **Two shapes, not one bag of optionals** — because there are two moments at
 * which this project writes a labels manifest and they know different things.
 *
 * Narrow on `batches === null`. Every reader that genuinely handles both does;
 * every reader that wants one says which in its own signature, so the compiler
 * refuses a `.version` read off a manifest that cannot have one.
 */
...
export interface LabelRun {
  labels: Record<string, string>;
  /**
   * **`CompletedLabelsFile`, not `LabelsFile`**, and the narrowing is the
   * declaration: a run that got this far made calls and knows both its prompt
   * version and its model, so the caller can read `version` and `generator`
   * off it without a guard — which is what `STEPS.labels`'s stamp does.
   * `PendingLabelsFile` is the other arm and no run ever produces one.
   */
  file: CompletedLabelsFile;
  batches: number;
